...
for ii=1:length(f)
  for jj=1:rdpsz(1)
    ...
    midSNR(ii,jj) = ( olf(costheta)*Sh0_model(f(ii)) )^2 ./ (sigma(radeccomb(jj,1))*sigma(radeccomb(jj,2)) + Sh0_model(f(ii))*(sigma(radeccomb(jj,1))+sigma(radeccomb(jj,2))) + Sh0_model(f(ii))^2*(1+olf(costheta)^2) );
  end
end
